...
plots_trait_a <- llply(unique(model_summary$trait),
                     function(x) {
                         ggplot(filter(model_summary, effect == 'a', trait == x), aes(loci, mean)) +
                             facet_wrap(~chrom, scale = 'free_x') +
                             geom_point() +
                             geom_errorbar(aes(ymin = lower, ymax = upper)) + theme_bw() + geom_hline(h = 0) + theme(title = element_text(x))
                     })

plots_trait_d <- llply(unique(model_summary$trait),
                     function(x) {
